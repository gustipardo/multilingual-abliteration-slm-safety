...
* **Side finding 1.** Hindi sits at the bottom of abliterated compliance at every size (39%, 65%, 60%) — lowest at E2B and 31B, third-lowest at E4B (behind Chinese and Arabic tied at 64%). The "low-resource means more vulnerable" assumption fails in this lineup. The spread Hindi vs the most-compliant language is widest at E2B (8 pp below Spanish) and tightens to within noise of the bottom cluster at the larger sizes.
...
* **Code:** [github.com/gustipardo/gemma4-abliteration](https://github.com/gustipardo/gemma4-abliteration)

## Figure 1

![Mean compliance vs Gemma 4 Dense size, base and abliterated, with non-monotonic abliterated curve peaking at E4B](https://raw.githubusercontent.com/gustipardo/gemma4-abliteration/main/figures/size_vs_compliance.png)
...
How strong this finding is depends on the size. At E2B, Hindi (39%) is 8 pp below the most-compliant language (Spanish, 47%) — a clear spread. At E4B and 31B the gap between Hindi and its neighbors at the bottom is within the pooled standard error (~1.8 pp), so Hindi blends into the bottom cluster rather than standing out as an outlier. In all three cases, Hindi is *not* the most vulnerable — the "low-resource = more vulnerable" prior fails at every size.
...
The gap could come from either of two things, and the compliance numbers alone can't tell them apart. 
- First, refusal geometry in Gemma 4 Dense may be less concentrated in a single direction than in the Yi, Qwen 2.5, Llama 3, and Gemma 2 models Wang et al. tested, so even a successful single vector ablation leaves some residual refusal in place. 
...
This pattern is consistent with the compliance peak. English abliteration relies on the seven languages sharing *one* refusal direction; that alignment is highest at E4B, where compliance is also highest. We cannot adjudicate causation from a single observation: the two peaks coincide, but we cannot tell whether one drives the other or both reflect a common cause.
...
The base-condition cells are not zero: 4.1% at E2B, 10.7% at E4B, 13.1% at 31B. We did not separate judge false positives (Claude Haiku has a small false-positive rate on borderline content, observed in an earlier E2B sanity check) from genuine compliance with harmful prompts. The headline (size-by-size comparison after abliteration) is unaffected: all three abliterated cells use the same judge, so the relative ordering across sizes is reliable.
