# Abliterated Gemma 4 Dense Complies with Harmful Prompts Most at 4B, Not at 31B ==WHY THIS TITLE? Reader has no context at beggiining and 4B and 31 B means nothing, more for everyone like bigger the model less secure, or something like that because finally the smallest model is the safetest Propose alternatives or somtheing like no matter the number of parameter democratized abliterated models with 60 complaiance rate athe free for everyone, compare how the other paper had to ablitarate the model them sled while this one are public==
...
* **Motivation.** The Gemma 4 Dense family from Google (released April 2, 2026) ships at three sizes ==They are really 4 models in Gemma 4, we avoid one anly because it has a different architecture== (E2B ~2.3B effective, E4B ~4.5B effective, 31B Dense), and `huihui-ai` released public abliterated versions of all three within 48 hours of the original release. Public abliterated checkpoints are the actual threat model: a non-expert user can download and run them. Whether the public recipe behaves the same way Wang et al.'s in-house extraction does, and how the effect scales inside a single Dense family, was open.
* **Main claim.** Across 7 languages and 100 BeaverTails harmful prompts per language, mean compliance after English abliteration is 42.9% at E2B, **68.1% at E4B**, and 64.4% at 31B. The size-to-compliance curve is not monotonic: it peaks at the mid-size model. The base-to-abliterated gap also peaks at E4B (+57.4 pp). ==Maybe I should make another run with the smallert model in Run pod.==
...
* **Mechanistic finding.** Cross-lingual cosine similarity of refusal directions also peaks at E4B (mean 0.37 vs 0.31 at E2B, 0.27 at 31B), and per-language Silhouette scores are highest at E2B (0.29) and lowest at 31B (0.23). Same-direction-at-every-scale: refusal geometry is most concentrated in a single cross-lingual direction at E4B and most distributed at 31B. The compliance peak and the geometry peak coincide. ==I dont undestand nothing of here, explain simplier== 
...
==Question Capability outpaces refusal between E2B and E4B, then refusal catches up at 31B. Anser: The judge (Claude Haiku) reads the full 512-token response. If E2B produces incoherent garbage, Haiku might still mark it as "complied" if it attempted to      
  answer rather than refuse. But this isn't measured directly — line 146 lists "we haven't separated judge-calibration noise from genuine instruction-following"
